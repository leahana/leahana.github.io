# optimize-doc 配套：Markdown 优化方法指南

本文档是 `optimize-doc` 拆分出的工作方法文档，用来说明
如何判断一篇 Markdown 文档该怎样重构结构、补充信息和提升可读性。

## 文档定位

- 角色：方法说明与工作流指南
- 适用阶段：已有初稿，准备重构结构或增强内容时
- 对应 skill 环节：分析原文、设计结构、补充图示与案例

## 与 skill 的关系

本文档是 `optimize-doc` 的公开配套文档之一。

- 面向对象：仓库协作者、博客维护者、需要理解优化思路的人
- 文档职责：解释判断原则、工作流和质量标准
- skill 职责：根据这些原则执行具体优化任务

## 适用时机

当你已经有一篇初稿，想继续增强结构、可读性和成品质量时，
优先看本文档。
...
对于简单的结构，可以使用 ASCII 艺术（使用 `text` 语言标识）：
...
1. **新文章创建时**：先从模板集合选结构，再回到本文档做增强
2. **优化现有文章**：按照 7步优化流程逐步改进
3. **发布前校验**：配合格式检查指南完成最后验收
...
| v1.1 | 2025-01-12 | 优化版本，保留核心内容和示例，优化结构以便模块化拆分 |
