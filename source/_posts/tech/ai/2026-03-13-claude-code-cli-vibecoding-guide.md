...
如果你上一次系统了解 Claude Code 还是这篇文章发布时的状态，那么
`2026-04-01` 到 `2026-04-08` 这一轮更新，最值得补课的是：
它开始把 `NO_FLICKER` 界面、插件、技能、Bedrock 接入、
`/resume` 恢复链路和多 Agent 管理做得更像一套可长期使用的工作流。

### 最新发布版本

- **最新 Release**：`v2.1.97`（`2026-04-08 21:52`）
- **一句话总结**：最近这一轮更新的核心，不是单个大功能，
  而是把日常使用中最容易卡住的几条主路径补齐了：
  无闪烁界面、会话恢复、插件执行、Bedrock 接入和权限控制。

### 这轮最值得关注的变化

| 版本 | 日期 | 我会怎么理解它 |
|------|------|----------------|
| `v2.1.97` | `2026-04-08` | `NO_FLICKER` 模式新增 `Ctrl+O` Focus View，状态栏支持 `refreshInterval` 周期刷新，`/agents` 会显示正在运行的子 Agent 数量；同时集中修了 `--resume`、权限、MCP reconnect、429 重试和 CJK 输入问题。 |
| `v2.1.96` | `2026-04-08` | 快速修复 `v2.1.94` 引入的 Bedrock `403 "Authorization header is missing"` 回归，说明 Bedrock 路线最近迭代很快。 |
| `v2.1.94` | `2026-04-07` | 新增 Amazon Bedrock Mantle 支持；API key、Bedrock/Vertex/Foundry、Team、Enterprise 用户默认 effort 从 `medium` 提高到 `high`；插件技能名优先使用 frontmatter 的 `name`。 |
| `v2.1.92` | `2026-04-04` | 新增交互式 Bedrock 配置向导；`/cost` 开始显示按模型和 cache hit 的拆分；`/release-notes` 变成交互式版本选择器；同时移除了 `/tag` 和 `/vim`。 |
| `v2.1.91` | `2026-04-02` | MCP 工具结果可以放宽持久化上限；新增 `disableSkillShellExecution`；插件现在可以自带 `bin/` 可执行文件并直接从 Bash tool 调用。 |
| `v2.1.90` | `2026-04-01` | 新增 `/powerup` 交互式教学入口；离线或 `git pull` 失败时可以保留旧 marketplace cache。 |
| `v2.1.89` | `2026-04-01` | `CLAUDE_CODE_NO_FLICKER=1` 正式提供无闪烁界面；`PreToolUse` hooks 新增 `defer`；`@` 提示开始支持命名 subagents。 |

### 认知同步：以前不能做，现在可以做什么

- **界面不只是“终端输出”了**：
  以前你更像是在看一串不断重绘的文本；现在 `NO_FLICKER`
  模式、`Ctrl+O` Focus View、状态栏刷新和更稳的 CJK 输入，
  说明 Claude Code 正在往“长期驻留工作台”演进。
- **插件开始具备真正的封装能力**：
  以前 plugin 更像“分发命令模板”；现在它已经能带 skill、frontmatter
  配置、`bin/` 可执行文件和更稳定的命名方式，越来越接近可复用工具包。
- **Bedrock 路线已经不是附带支持**：
  Bedrock 向导、Mantle 支持、权限和鉴权修复都在密集迭代。
  如果你走企业云平台接入，最近这几版值得重点关注。
- **`/resume` 正在变成主工作流而不是补充能力**：
  最近多个版本都在修 `--resume`、prompt cache、transcript、
  worktree 和 picker 的问题，说明官方在把“长会话恢复”当成核心体验来打磨。
...
它会打开 Focus View，把 prompt、工具摘要和最终响应整理成更易扫读的结构。
...
这一组命令基本覆盖了“学功能、看版本、看成本、调推理强度”的主路径。
...
然后在登录界面选择 `3rd-party platform`，走交互式 Bedrock 配置向导。

### 主分支未发版变更

- **本次未单列**：截至 `2026-04-09` 检查 `main` 分支顶部提交，
  可见的最近提交主要是 `chore: Update CHANGELOG.md`。
- **状态说明**：这说明最近可确认的公开变化基本都已经体现在 release /
  changelog 中；本次不额外写“已合并未发版功能”，避免把线索误记成正式能力。
...
- [GitHub - anthropics/claude-code commits/main](https://github.com/anthropics/claude-code/commits/main)
